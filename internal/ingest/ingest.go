...
//
// When cfg.Bench is set, the worker additionally records per-block timing
// (mptStart, waitForCommitments, completed) and writes a row to blocks.csv.
	...
	for blockInfo := range blockInfoCh {
		...
		if buf, ok := buffered[blockInfo.blockNumber]; ok {
			for _, u := range buf {
				...
				// tr.MustUpdate(u.address.Bytes(), u.commitment.Bytes())
				// tr.MustUpdate(u.address.Bytes(), u.commitment.Bytes())
				pending--
			}
		...
		// Measure the actual time spent blocking on the channel (excluding
		// trie application) as wait-for-commitments latency.
		...
		for pending > 0 {
			...
			u := <-commitCh
			if bench != nil {
				waitCommitmentsNs += time.Since(t).Nanoseconds()
			}

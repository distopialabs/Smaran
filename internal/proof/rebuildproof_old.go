...
func OldRebuildSegmentTreeForProof(account common.Address, lxRequiredBatchIdxs map[uint64][]uint64, startingVersion uint64, endingVersion uint64, db *db.SamuraiStore, precomputedData *config.PrecomputedData) (map[string]tree.BatchTree, []*tree.HistoricalBalance) {
	...
	for version := uint64(0); version < cbInfo.Version; version++ {
		...
		OldAddLeafNode(accountInfo, hbInfo.Version, hbInfo.Hash())
	...
	for layer := uint64(2); layer <= MaxLayer; layer++ {
		for _, batchIdx := range lxRequiredBatchIdxs[layer] {
			...
			OldInsertCommitmentHashes(layer, batchIdx, &treeBatch, account, cbInfo.Version, db)
...
func OldAddLeafNode(accountInfo *tree.AccountInfo, leafNodeIdx uint64, leafNodeHash common.Hash) {
	...
	for layer := uint64(1); layer <= MaxLayer; layer++ {
		OldUpdateLXTree(accountInfo, lxBatchLeafNodeOffsetIdx(layer), lXm1RootHash, layer)
...
func OldInsertCommitmentHashes(layer uint64, batchIdx uint64, batchTree *tree.BatchTree, account common.Address, latestVersion uint64, sdb *db.SamuraiStore) {

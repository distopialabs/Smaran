...
func OldNewCache(db *pebble.DB, maxEntriesCount, maxDirtyEntriesCount, maxUpdatesSinceLastFlush uint64, flushInterval time.Duration, precomputedData *config.PrecomputedData) *OldCache {
	cache := &OldCache{
		// TODO: tune this
...
func (c *OldCache) Get(id common.Address) (*AccountInfo, error) {
	c.mu.Lock()
...
func (c *OldCache) Set(id common.Address, accountInfo *AccountInfo) {
	c.mu.Lock()
	e := c.entries[id]
...
func (c *OldCache) flushOne(e *CacheEntry) error {
	e.mu.Lock()
...
func (c *OldCache) flushSome(full bool) {
	c.mu.Lock()
	start := time.Now()
...
func (c *OldCache) attach(e *CacheEntry) {
	if c.head == nil {
		c.head = e
		c.tail = e
	} else {
...
func (c *OldCache) detach(e *CacheEntry) {
...
func (c *OldCache) moveToHead(e *CacheEntry) {

...
func benchIngestCmd() *cli.Command {
	return &cli.Command{
		...
		Flags: []cli.Flag{
			&cli.StringFlag{Name: "db-dir", Value: "/data/local/tmp/bench-verklekzg", Usage: "Path to persistent DB directory"},
		...
		Action: func(c *cli.Context) error {
			...
			csvPath, err := benchutil.IngestionOutputPath(outputDir, "verklekzg", kUsers)
			if err != nil {
				return err
			}
			updateMetricsPath, err := benchutil.UpdateMetricsOutputPath(outputDir, "verklekzg", kUsers)
			if err != nil {
				return err
			}

	...
	for range fetchWorkerCount {
		...
		go func() {
			...
			for {
				...
				modifiedAccounts, err := ledger.GetModifiedAccountsByNumber(bn, client)
				if err != nil {
				...
				balances, err := ledger.BatchMultiUserBalance(modifiedAccounts, bn, client)
				if err != nil {
...
func sanityCheck(testBlock uint64, client *rpc.Client) {
	actualModifiedAccounts, err := ledger.GetModifiedAccountsByNumber(testBlock, client)
	if err != nil {
		panic(fmt.Errorf("failed to get modified accounts by number %d: %w", testBlock, err))
	}
	actualBalances, err := ledger.BatchMultiUserBalance(actualModifiedAccounts, testBlock, client)
	if err != nil {

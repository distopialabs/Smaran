#!/usr/bin/env python3
"""
plot_shards_throughput.py — Throughput per user across shards for a single protocol.

For each shardN in logs/shards/microbench_shards_output/, finds the newest
ingestion CSV for each user count, computes throughput/user, and plots a
grouped bar chart (one group per user count, one bar per shard).
"""
...
def plot_bar_chart(
    ...
    style_label = "per_user" if args.per_user else "total"
    fname = f"shards_throughput_{protocol}_{style_label}"
...
def main():
    ...
    parser.add_argument("--skip-users", action="append", type=int, default=[],
                        metavar="N",
                        help="User count to exclude (repeatable).")

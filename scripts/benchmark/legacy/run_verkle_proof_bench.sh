#!/bin/bash
#
# Verkle proof benchmark sweep against a running verkle gRPC server.
#
# For each (range_size, duration) pair (same index in RANGE_SIZES and DURATIONS),
# runs verkle-proofc bench, then sleeps for cooldown. Unlike scripts/benchmark/run_openloop_bench.sh,
# this does not start a server; point SERVER_ADDR at your verkle serve instance.
#
# Summaries are written under OUTPUT_DIR (see benchutil / verkle-proofc --help).

set -euo pipefail

# --- Configuration ---
CLIENT_BIN=${CLIENT_BIN:-./bin/verkle-proofc}
...
RANGE_SIZES=(${RANGE_SIZES:-1 100 500 1000 5000 7000})
DURATIONS=(${DURATIONS:-2m 2m 2m 5m 5m 5m})

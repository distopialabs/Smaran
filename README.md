...
| **C. Your own server(s)** | One or two Ubuntu 22.04 machines; see [Install](#install-paths-bc--one-time). KT needs two nodes; DL also runs single-machine. DL needs the dataset ([Zenodo DOI 10.5281/zenodo.21317398](https://doi.org/10.5281/zenodo.21317398)); KT needs no dataset |
...
The rest of this README: [Quick start](#quick-start-cloudlab-profile) for the
profile path, then the **Decentralized Ledger** guide in depth. The
equivalent Key Transparency guide is
[docs/kt-artifact-guide.md](docs/kt-artifact-guide.md).
...
Without `/local/cluster.env` (manual/own-server setups) the same scripts run everything on the

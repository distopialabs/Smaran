...
def _parse_cauchy_txt(txt_path: Path) -> Dict[str, float]:
    ...
    Keys: throughput_rps, avg_proofgen_ms, avg_response_ms, avg_verify_ms
    """
    data: Dict[str, float] = {}
    with txt_path.open(encoding="utf-8") as f:
        for line in f:
            ...
            elif line.startswith("Avg Proofgen:"):
                data["avg_proofgen_ms"] = _parse_duration_ms(line.split(":", 1)[1].strip())
...
def parse_cauchy_csv(csv_path: Path) -> List[ProofBenchmarkPoint]:
    ...
    # Build range_size -> metrics dict from the .txt files
    ...
    with csv_path.open(encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            points.append(ProofBenchmarkPoint(
                ...
                avg_proofgen_ms=m.get("avg_proofgen_ms", 0.0),
...
def _rps_formatter(x, pos):
    # if x <= 0.001:
    #     # Use 10^-n notation
    #     return f"\\textbf{{$\mathrm{{10^{{{int(math.log10(x))}}}}}$}}"
